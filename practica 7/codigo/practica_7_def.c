...
Project : Práctica 7 'Contador de 00 a 99'
Version : 1.0
Date    : 16/02/2026
...
void main(void) {
    ...
    while (1) {
        ...
        // Si se detecta un flanco de subida de 1 a 0
        if (estado_boton_actual && !estado_boton_anterior) {
            unidad++;
            delay_ms(40);
        }

        // Si se detecta un flanco de bajada de 0 a 1
        if (!estado_boton_actual && estado_boton_anterior) {
            delay_ms(40);
        }

        // Si la unidad llega a 10 se reinicia y se incrementa la decena
        if (unidad == 10) {
            unidad = 0;
            decena++;
        }

        // Si la decena llega a 10, se llego a 99 + 1, este se reinicia
        if (decena == 10) {
            decena = 0;
        }
