...
Project : Práctica 3 'Convertidor BCD a 7 Segmentos' y 'Contador Hexadecimal'
Version : 1.0
Date    : 10/02/2026
...
void main(void) {
    ...
    while (1) {
        ...
        valor_entrada = PIND & 0x0f; // Enmascarar los 4 bits menos significativos

        if (MODO) {
            if (valor_entrada < 10) {
                  PORTB = bcd[valor_entrada];
                  PORTA = ~bcd[valor_entrada];
            } else {
            ...
            PORTB = hex[valor_entrada];
            PORTA = ~hex[valor_entrada];
        }
    }
}
