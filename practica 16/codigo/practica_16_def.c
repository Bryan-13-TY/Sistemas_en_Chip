...
Project : Práctica 16 'Termómetro 0 a 50 °C (32 a 122 °F)'
Version : 1.0
Date    : 13/03/2026
...
unsigned char cn, intensidad;
unsigned char unidades, decenas, centenas;
bit isFahrenheit = 0;
bit pasadoCF = 1, actualCF;
bit pasadoMODE = 1, actualMODE;
const char tabla7segmentos[10] = {0x3F, 0x06, 0x5b, 0x4f, 0x66, 0x6d, 0x7d, 0x07, 0x7f, 0x6f};

unsigned char modo = 0;       // 0:Real-time, 1:Max, 2:Min
...
#define LED_GREEN PORTC.4
#define LED_BLUE PORTC.5
#define BTN_CF PIND.0
#define BTN_MODE PIND.1
#define C0 PORTC.0
#define C1 PORTC.1
#define C2 PORTC.2 
...
void main(void) {
    ...
    while (1) {
        ...
        cn = read_adc(1) * 500 / 255;

        // Manejo pulsador Celsius/Fahrenheit
        actualCF = BTN_CF;
        if (pasadoCF && !actualCF) {
            isFahrenheit = !isFahrenheit;
            delay_ms(20);
        }
        pasadoCF = actualCF;

        // Manejo pulsador de modo
        actualMODE = BTN_MODE;
        if (pasadoMODE && !actualMODE) {
            modo = (modo + 1) % 3;
            max_temp = 0;       // Resetear m�ximos
            min_temp = 0xFFFF;  // Resetear m�nimos
            delay_ms(20);
        }
        pasadoMODE = actualMODE;

        // Determinar valor a mostrar seg�n modo
        switch(modo) {
            case 0: // Tiempo real
                intensidad = isFahrenheit ? (9 * cn /5 +32) : cn;
                LED_BLUE = 1;
                LED_GREEN = 0;
                LED_RED = 0;
                break;
                
            case 1: // M�xima temperatura
                if(cn > max_temp) max_temp = cn;
                intensidad = isFahrenheit ? (9 * max_temp /5 + 32) : max_temp;
                LED_BLUE = 0;
                LED_GREEN = 1;
                LED_RED = 0;
                break;
                
            case 2: // M�nima temperatura
                if(cn < min_temp) min_temp = cn;
                intensidad = isFahrenheit ? (9 * min_temp /5 + 32) : min_temp;
                LED_BLUE = 0;
                LED_GREEN = 0;
                LED_RED = 1;
                break;
        ...
        // Convertir intensidad a d�gitos
        centenas = intensidad / 100;
        decenas = (intensidad % 100) / 10;
        unidades = intensidad % 10;
