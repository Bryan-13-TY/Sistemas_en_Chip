...
Project : Práctica 8 'Cronómetro de 59.9 segundos'
Version : 1.0
Date    : 17/02/2026
...
#include <delay.h>
#include <stdbool.h>
#define RESET_BUTTON PIND.0
#define STOP_BUTTON PIND.1
#define START_BUTTON PIND.2

// Declare your global variables here

void main(void) {
    // Declare your local variables here
    unsigned char decena = 0, unidad = 0, decima = 0;
    bool debe_avanzar = false;
    const char bcd[10] = {0x7e, 0x0c, 0xb6, 0x9e, 0xcc, 0xda, 0xfa, 0x0e, 0xfe, 0xde};
    ...
    while (1) {
        ...
        if (!START_BUTTON) debe_avanzar = true;
        if (!STOP_BUTTON) debe_avanzar = false;
        
        if (!RESET_BUTTON) {
            decena = 0; unidad = 0; decima = 0;
            debe_avanzar = false;
        }

        // Reiniciar las decenas
        if (decena == 6) debe_avanzar = false;

        if (debe_avanzar) {
            decima++;

            // Reiniciar los decimales y aumentar unidades
            if (decima == 10) {
                decima = 0;
                unidad++;
            }

            // Reiniciar las unidades y aumentar decenas
            if (unidad == 10) {
                unidad = 0;
                decena++;
            }
        ...
        // Obtener los números hexadecimales
        PORTB = bcd[decena];
        PORTA = bcd[unidad];
        PORTC = bcd[decima];
        delay_ms(100);
